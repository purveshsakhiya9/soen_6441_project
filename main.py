...
drop_engine = "DROP TABLE IF EXISTS engine"
drop_body = "DROP TABLE IF EXISTS body"
drop_color = "DROP TABLE IF EXISTS colour"
drop_mileage = "DROP TABLE IF EXISTS mileage"


cursor.execute(drop_engine)
cursor.execute(drop_body)
cursor.execute(drop_color)
cursor.execute(drop_mileage)
cursor.execute(drop_vehicle)
...
for item in car_data:
	...
	cursor.execute(
		...
		(engine_id,engine_type,fuel_type,cylinders,size,horsepower_hp,horsepower_rpm,cam_typ,transmission,e_id)
	)
	connection.mydb1.commit()
	# mileage
	m_id = item['id']
